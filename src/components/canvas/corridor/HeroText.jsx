 ...
 * - SAKSHAM in Rubik Scribble font (splits into letters during scroll)
...
const HeroText = ({ position = [0, 0.3, 0] }) => {
    ...
    // Letter positions for SAKSHAM split effect (auto-spaced for any word length)
    const letters = useMemo(() => {
        const word = 'SAKSHAM';
        const chars = word.split('');
        const n = chars.length;
        const range = 1.9; // total X spread, matches original design
    ...
    const taglineWords = useMemo(() => [
        ...
        { text: 'AI/ML', baseX: -0.4, splitDir: -0.8, delay: 0 },
        { text: 'enthusiast', baseX: 0.4, splitDir: 0.8, delay: 0 },
        { text: '/>', baseX: 0.85, splitDir: 1.5, delay: 0 },
    ], []);
        ...
        <group ref={groupRef} position={position} scale={[scale, scale, 1]}>
            ...
            {letters.map((letter, i) => (
                <Text
                    ...
                    fontSize={0.9 * Math.min(1, 4 / letters.length)}
